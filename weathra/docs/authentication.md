...
| `user_plans` | user-owned | `0006`: RLS enabled and forced, with an owner-read policy. `0015` adds owner-scoped `INSERT` and `UPDATE` — tiers became self-selectable on 2026-09-13 — each carrying `WITH CHECK (user_id = weathra_current_user_id())`, so a caller writes their own row and cannot write another's. `DELETE` is still withheld: choosing Free is an update, which keeps `assigned_by` and `assigned_at`. Assigning *somebody else* a tier remains privileged and audited |

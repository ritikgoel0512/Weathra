...
**Approved was not built, at the time this was written; all seven have since been built.** At the time of writing all seven routes stated that their screen was not yet available, and the
...
|---|---|
| "SUBSCRIPTION ID WX-PRO-8891-BETA", "BILLING INTERVAL Monthly Professional", "NEXT BILLING DATE", "PAYMENT METHOD Visa ending in 4421", "Manage Payment Information", "DOWNLOAD INVOICES", "UPGRADE PLAN" | **Nothing, and this refusal stands.** Weathra bills nobody: `subscription_plans` carries an unused external reference so a payment integration has somewhere to land later, and there is no interval, no invoice and no card. Since 2026-09-13 a person *can* change their own tier (task 34.44) — but that is a selection, not a purchase: no price is published and nothing is charged, so none of the billing content above became drawable. Drawing it would be an invented commercial relationship — the largest single refusal in the set. |
...
| 2026-09-10 | `10-plan-usage.png` | The billing half of the artifact is not implemented — no subscription id, interval, next billing date, payment method, invoice or upgrade control — and the recent-activity panel is a summary rather than the seven-day line chart | §5 already refuses the billing content as an invented commercial relationship: Weathra bills nobody, and the self-service tier selection added on 2026-09-13 charges nothing and publishes no price, so it did not make any of that content drawable. The chart is refused for a narrower reason — `/me/usage` reports `recent` as one summary over N days, not a per-day series, so a line would be drawn from points the backend does not send. The vector-node, rate-limiter and per-user export panels are refused in §5 and absent. Task 34.10. |

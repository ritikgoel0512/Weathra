...
| 1.6 | The hero is far less dominant than the artifact's: the temperature is not the largest thing on the screen, and there is no metric cluster | C | **not done** |
| 1.7 | "Brief me on a place" is a form above the fold; the artifact has only the top-bar search | C | **not done** |
| 1.8 | Day cards carry no condition glyph; the artifact's carry one each | B | **not done** |
...
| 3.3 | Six bare `<input type="date">`, a `<select>` and a number field open the screen; the artifact has one date-range control, a °C/°F toggle and an Export action | C | **not done** |
| 3.4 | Metric tiles carry no icon and no delta caption; five of seven wrap awkwardly | B | **not done** |
| 3.5 | The left column ends well above the right, leaving a tall void | B | **not done** |
| 3.6 | No unit toggle and no export control | B | **not done** |
...
| 4.3 | Ranking cards state the same figure twice — once as the headline, once as a "mean · Temperature" row | B | **not done** |
| 4.4 | The comparison chart draws flat full-height bars in the ANALYTICS violet, used as a series colour | B | **not done** |
| 4.5 | The query form stays at full height above the results; the artifact replaces it with a compact toolbar | C | **not done** |
| 4.6 | "How this comparison was made" is a table of machinery, partly "Not reported" | C | **not done** |
...
| 5.5 | Left column ends well above the right | B | **not done** |
...
| 7.2 | Native `<select>` chrome against the artifact's styled controls | B | **not done** |
| 7.3 | "Your choice." repeated under each control | B | **not done** |
| 7.4 | Tabs carry no icons | B | **not done** |
...
Named plainly, because the brief asked for category B fixed and category C redesigned, and the items
below are neither done nor abandoned. None is a regression; each is a drift the audit found and left.

1. **The Dashboard hero** (1.6) and **its briefing form** (1.7) — the artifact's most dramatic band
   is production's quietest. This is a layout rebuild, not an edit.
2. **The Historical Analytics and Compare Cities query forms** (3.3, 4.5) — six native date inputs
   and a bare select where the artifacts have a compact toolbar. The largest single remaining source
   of the console character.
3. **Native `<select>` and `<input type="date">` chrome** (3.3, 7.2) across three screens.
4. **Column-height imbalance** on Historical Analytics, Compare Cities and Agent Evidence (3.5, 4.3,
   5.5).
5. **Chart colour** on Compare Cities (4.4) — the ANALYTICS violet used as a data-series colour.
6. **The interpretation footer's inapplicable fields** (2.11). Left deliberately: `AttributionFooter`
   renders provider, location, period and retrieval on every surface because `specs/web-ui` requires
   it on every weather-bearing surface, and narrowing that primitive on this audit's own judgement
   is a change to a guarantee rather than to a layout. It wants a decision, not a patch.
7. **Small text items**: repeated "Your choice." (7.3), missing tab icons (7.4), missing day glyphs
   (1.8), the duplicated ranking figure (4.3).
8. **A photographed refused-credential sign-in** (8.5) — the stub cannot produce one.

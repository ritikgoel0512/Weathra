...
- **Backend-controlled model policy layer** between the LangGraph orchestrator and the LLM client. Named, subscription-aware policies — `free_default`, `balanced`, `high_reasoning`, `admin_experimental` — resolve a model per call from the acting principal's backend-derived plan and the call role. Plans map onto Free / Plus / Pro. The frontend is never trusted to authorize premium model access; enforcement is server-side, and a caller-named model is advisory at most.
...
| **Model policy and plan tiers** — server-side model routing by subscription plan, with per-call telemetry and quotas | **MVP**, backend-enforced | Free / Plus / Pro assigned administratively; no payment processing in this change |
